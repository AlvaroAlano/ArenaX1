...
const handleReport = async (result: 'win' | 'loss') => {
  const confirmed = await confirmAction({
    title: `Confirmar ${result === 'win' ? 'vitória' : 'derrota'}?`,
    message: 'Isso é irreversível e declarações falsas podem resultar em banimento.',
    confirmLabel: result === 'win' ? 'Confirmar Vitória' : 'Confirmar Derrota',
    danger: true,
  })
  if (!confirmed) return
...
<template>
  <div class="flex-1 p-6 md:p-10 max-w-7xl mx-auto w-full">
    ...
    <div v-else-if="challenge" class="space-y-8">
      ...
      <!-- Banner de Status -->
      <div
        class="rounded-lg p-4 text-center border font-bold uppercase tracking-widest text-sm bg-surface-2 border-hairline"
        :class="{
          'text-ink-muted': challenge.status === 'in_progress',
          'text-semantic-success': challenge.status === 'completed',
          'text-semantic-danger': challenge.status === 'disputed'
        }"
      >
      ...
      <!-- Placar e Versus -->
      <div class="bg-surface-1 border border-hairline rounded-xxl p-8 relative overflow-hidden">
        <!-- Detalhes do Pote -->
        <div class="absolute top-0 left-1/2 -translate-x-1/2 bg-surface-2 border border-hairline border-t-0 px-8 py-3 rounded-b-xxl flex items-center gap-4">
          <span class="text-ink-subtle text-caption font-bold uppercase tracking-wider">Aposta</span>
          <span class="text-ink font-semibold text-xl">{{ formatCurrency(challenge.bet_amount) }}</span>
        </div>
        ...
        <div class="flex flex-col md:flex-row items-center justify-between gap-10 mt-12">
          ...
          <div class="flex-1 text-center space-y-4">
            <div class="w-24 h-24 mx-auto rounded-full bg-surface-3 border-4 border-hairline flex items-center justify-center">
          ...
          <div class="flex flex-col items-center">
            <div class="w-16 h-16 rounded-full bg-primary flex items-center justify-center">
              <span class="text-on-primary font-semibold italic text-xl">VS</span>
            </div>
          </div>
      ...
      <div v-if="challenge.status === 'in_progress' && (isCreator || isOpponent)" class="bg-surface-1 border border-hairline p-8 rounded-xxl text-center">
        ...
        <div v-else>
          ...
          <div class="flex flex-col sm:flex-row justify-center gap-6">
            <button 
              ...
              :disabled="reporting"
              class="flex-1 max-w-xs bg-semantic-success hover:opacity-90 text-on-primary font-semibold text-lg py-5 rounded-lg transition-opacity disabled:opacity-50 flex items-center justify-center gap-2"
            >
            ...
            <button 
              ...
              :disabled="reporting"
              class="flex-1 max-w-xs bg-semantic-danger hover:opacity-90 text-on-primary font-semibold text-lg py-5 rounded-lg transition-opacity disabled:opacity-50 flex items-center justify-center gap-2"
            >
              <svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
