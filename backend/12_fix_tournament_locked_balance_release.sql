...
--
-- Dois ajustes no mesmo arquivo (ainda não rodado, então vão direto na
-- versão final das funções em vez de em arquivos separados):
--
-- 1) BUG real encontrado testando o fluxo do admin ponta a ponta: quando um
--    torneio online termina (final resolvida, disputa de 3º lugar resolvida,
--    ou alguém eliminado numa rodada sem chance de 3º lugar), o valor da
--    inscrição de CADA participante nunca saía de `locked_balance`. O prêmio
--    em si sempre foi pago certinho (`balance` sobe certo) — só o saldo
--    travado ficava PRESO pra sempre, inflando "Saldo travado" na Carteira
--    de cada jogador e no portal de admin, a cada torneio concluído.
--    Regra de quando liberar (mesma ideia de fn_report_challenge_result):
--      * Final resolvida -> libera dos DOIS finalistas (campeão e vice).
--      * Disputa de 3º lugar resolvida -> libera dos DOIS (3º e 4º lugar).
--      * Eliminado numa rodada ANTERIOR à semifinal (só existe em torneios
--        de 8/16, sem chance de disputar 3º lugar) -> libera na hora, sem prêmio.
--      * Quem ainda está vivo na chave NÃO libera ainda — continua "em jogo"
--        até ser eliminado ou a final acabar.
--
-- 2) Decidido em conversa com o usuário: em torneios de 8/16 jogadores
--    (não em torneios de 4, que já reparte entre 3 dos 4 jogadores), o 4º
--    lugar agora recebe de volta exatamente o valor da própria inscrição
--    (empate, sem lucro) em vez de perder tudo — fica mais competitivo pra
--    quem cai na semifinal. O dinheiro sai proporcionalmente do pote de
--    1º/2º/3º (a comissão de 10% da ArenaX1 não muda): calcula-se um
--    "pote ajustado" = pote líquido − 1 taxa de inscrição, e os 50/30/20%
--    de sempre passam a incidir sobre esse pote ajustado (menor) em vez do
--    pote líquido cheio. Em torneios de 4 jogadores nada muda (pote
--    ajustado = pote líquido, 4º lugar continua sem prêmio).
