...
--
-- Objetivo: nova regra de premiação do Torneio Online Pago, decidida em
-- conversa com o usuário — substitui o antigo esquema fixo "50% / 30% / 20%
-- + 4º lugar recebe a própria inscrição de volta" por uma tabela por
-- tamanho de chave, sempre em cima do pote líquido (depois do rake de 10%,
-- que não muda):
--
--   4 jogadores  — só o 1º leva: 100%
--   8 jogadores  — top 3: 55% / 30% / 15%
--   16 jogadores — top 4: 50% / 25% / 15% / 10%
--
-- O 4º lugar em torneio de 16 deixa de ser "reembolso da inscrição" e passa
-- a ser um PRÊMIO de verdade (10% do pote líquido, tipo 'tournament_prize',
-- não mais 'bet_refund') — por isso ganha uma coluna própria em
-- `tournaments` (mesmo padrão de third_place_participant_id).
--
-- CREATE OR REPLACE é seguro de rodar em cima do 13 (mesmas assinaturas de
-- função, só o corpo muda).

-- ─────────────────────────────────────────────────────────────────────────
-- 1) Coluna nova pra registrar o 4º colocado (agora prêmio, não reembolso).
-- ─────────────────────────────────────────────────────────────────────────
alter table public.tournaments
  add column if not exists fourth_place_participant_id uuid references public.tournament_participants(id);

-- ─────────────────────────────────────────────────────────────────────────
-- 2) Reportar resultado por consenso: mesmo corpo do 13, com a tabela de
--    percentuais nova no lugar do "adjusted_pool - reembolso do 4º".
-- ─────────────────────────────────────────────────────────────────────────
...
BEGIN
  ...
  -- já descontado o rake de 10% — ver 19_tiered_prize_distribution.sql).
...
-- ─────────────────────────────────────────────────────────────────────────
-- 3) Resolver disputa (admin): mesma tabela de percentuais, espelhando a
--    função acima.
-- ─────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────
-- Grants (defensivo — mesmas assinaturas, já concedidas, reafirmando).
-- ─────────────────────────────────────────────────────────────────────────
